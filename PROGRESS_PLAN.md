...
All deployment-specific addresses, origins, image names, and chain anchors are parameters. All sensitive values are absent from Bicep/parameter files and represented only by versionless Key Vault secret references. A new deployment requires exact HTTPS origins, chain/network 1952, the official test USDt0 asset, nonzero and distinct registry/payout/controlled-provider roles, the exact 40-character current source commit, and all five images from the selected existing ACR in `<repository>:<full-commit>@sha256:<registry-digest>` form. Later read-only audit accepts only a reachable immutable deployed backend commit, so an accepted deployment remains verifiable after a documentation-only follow-up commit. Bicep itself also fails closed on ACR mismatch, commit/digest mismatch, duplicate fixture providers, unsafe payment caps, enabled budget without contacts, and active backend without an approved cutover.
...
After this record was committed, a read-only audit exposed a tooling-only provenance issue: the active parameter file correctly names accepted backend commit `e19d16820b99b9424bf9debedb3819910403999d`, while the checked-out `HEAD` is this later documentation commit. Validation now distinguishes the two safe operations: `what-if.sh` and `deploy.sh` require the parameter `buildCommit` to equal the checked-out `HEAD`, while standalone read-only artifact validation requires that it be a reachable immutable ancestor. The active parameter file passed the latter validation and all five configured ACR digests were rechecked. No deployment command, live configuration change, payment, or chain call was made for this correction.
