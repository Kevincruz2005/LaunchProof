...
The backend and controlled fixtures have independent immutable provenance. `buildCommit` must equal the checked-out backend commit, while `fixtureBuildCommit` identifies the previously built signed fixture artifact. Each image must carry its corresponding full commit tag and an ACR-provided `sha256` digest. Container App revision suffixes use the deployment commit, so an accepted fixture artifact can be rolled out again without reusing a historical Azure revision name. This permits a backend-only fix without silently changing the signed Launch Contract identity of an accepted fixture.
